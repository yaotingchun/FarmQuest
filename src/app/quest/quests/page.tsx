...
function QuestsContent() {
  ...
  useEffect(() => {
    ...
    const planFromUrl = selectedPlanType || 'Budget'
  ...
  const MainContent = () => {
    ...
    if (viewMode === 'list' || !activePlant) {
      const allTasksWrapped = getAllTasksDueToday(visiblePlants, getQuestPlant)
    ...
    const isSearching = activePlant.source_category === 'posted_order' && (!activePlant.order_status || activePlant.order_status === 'open')
    ...
    if (isSearching) {
      return (
        <div className="quest-main-content" style={{ maxWidth: '920px', margin: '0 auto', width: '100%' }}>
          ...
          <div className="quest-hub-no-tasks" style={{ padding: '6rem 2rem', textAlign: 'center' }}>
            <div style={{ fontSize: '4rem', marginBottom: '1.5rem', animation: 'float 3s ease-in-out infinite' }}>🔍</div>
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '1rem' }}>Farmer Search in Progress</h2>
            <p style={{ color: 'var(--text-secondary)', maxWidth: '400px', margin: '0 auto 2.5rem', lineHeight: 1.6 }}>
              We&apos;ve posted your order to the marketplace. Once a farmer accepts, your planting journey will begin here!
            </p>
            <div className="quest-searching-badge" style={{ padding: '8px 20px', fontSize: '0.85rem' }}>
              Current Status: Open for Bidding
            </div>
          </div>
        </div>
      )
      ...
      <div className="quest-main-content" style={{ maxWidth: '920px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <PlantStatusCard
            ...
            orderStatus={activePlant.order_status}
