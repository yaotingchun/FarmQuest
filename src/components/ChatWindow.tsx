...
export default function ChatWindow({ orderId, isFarmer, status, onStatusUpdate }: ChatWindowProps) {
  ...
  const fetchMessages = async () => {
    try {
      const res = await fetch(`${API_URL}/api/marketplace/orders/${orderId}/messages`)
      if (res.ok) {
  ...
  const handleSend = async (e?: React.FormEvent, customText?: string) => {
    ...
    try {
      await fetch(`${API_URL}/api/marketplace/orders/${orderId}/messages`, {
        method: 'POST',
  ...
  const handleAction = async (actionType: string, nextStatus: string) => {
    try {
      // 1. Send status update to backend with user context
      ...
      await fetch(`${API_URL}/api/marketplace/orders/${orderId}/messages`, {
        ...
        body: JSON.stringify({
          sender_uid: user?.uid,
          sender_name: profile?.username || 'User',
          text: `Marked order as ${nextStatus}|${status}`,
          type: 'action',
    ...
    <div className="chat-window">
      <div className="chat-header">
        ...
        <div className="chat-actions-bar">
          {isFarmer && status === 'accepted' && (
            <button className="chat-wf-btn" onClick={() => handleAction('planting', 'planting')}>
              <Sprout size={14} /> Start Planting
            </button>
          )}
          {isFarmer && status === 'planting' && (
            <button className="chat-wf-btn" onClick={() => handleAction('growing', 'growing')}>
              <Leaf size={14} /> Mark as Growing
            </button>
          )}
          {isFarmer && status === 'growing' && (
            <button className="chat-wf-btn" onClick={() => handleAction('harvested', 'harvested')}>
              <CheckSquare size={14} /> Mark Harvested
            </button>
          )}
          {isFarmer && status === 'harvested' && (
            <button className="chat-wf-btn" onClick={() => handleAction('delivering', 'delivering')}>
              <Truck size={14} /> Out for Delivery
            </button>
          )}
          {!isFarmer && status === 'delivering' && (
            <button className="chat-wf-btn success" onClick={() => handleAction('completed', 'completed')}>
